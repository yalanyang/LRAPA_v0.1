...
``` shell
lrapa couplingTSS -i test.mapping.bam -s TSS.ref.txt -p test.PAS.bed -o TSS-PAS.coordination.txt
```
...
-   `-s, --tss`: Reference annotation TSS file (required).
...
``` shell
lrapa couplingExon -i test.full_length.bam -s SE.ref.txt -p test.PAS.bed -o PAS-exon.coordination.txt
```
...
-   `-s, --se`: Reference annotation SE file (required).
