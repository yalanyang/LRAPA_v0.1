##05/28/2026 updated by Yalan Yang
##using WARM and MPRO values to quantify PA usage. 
...
#setwd("/Users/yangyalan/results/Long-read-APA/2026_NC_revision/Diff/Fetal_brain/bulk")
##Usage: Rscript 4.1_diff.DRIMSeq_0.3_2.R -i human_fetal.brain.count.txt -s sample.txt -c CP -n GZ
...
if (method=="gene") {
  ...
  colnames(counts)[1] <- "gene_id"
  colnames(counts)[2] <- "feature_id"
  counts <- counts %>% group_by(feature_id) %>% summarise(across(all_of(sample_id), sum), .groups = 'drop')
