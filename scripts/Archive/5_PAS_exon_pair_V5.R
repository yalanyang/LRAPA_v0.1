...
chisq_test<- function(counts) {
  ...
  chisq_results <- lapply(gene_matrices, function(df) {
    matrix <- as.matrix(df[, c("in_in", "in_out")])
    rownames(matrix) <- df$tes_id
    if (nrow(matrix) >= 2 && all(rowSums(matrix) > 0)) {
      test <- chisq.test(matrix)
      contrib <- (test$observed - test$expected)^2 / test$expected
      max_idx <- which(contrib == max(contrib, na.rm = TRUE), arr.ind = TRUE)[1, , drop = FALSE]
      min_idx <- which(contrib == min(contrib, na.rm = TRUE), arr.ind = TRUE)[1, , drop = FALSE]
      max_transcript <- rownames(contrib)[max_idx[1]]
      min_transcript <- rownames(contrib)[min_idx[1]]
      
      max_pas_id <- df$pas_id[df$tes_id == max_transcript][1]
      min_pas_id <- df$pas_id[df$tes_id == min_transcript][1]
      
      max_contrib_val <- contrib[max_idx[1], max_idx[2]]
      min_contrib_val <- contrib[min_idx[1], min_idx[2]]
      
      max_pas_id <- df$pas_id[df$tes_id == max_transcript][1]
      min_pas_id <- df$pas_id[df$tes_id == min_transcript][1]
      list(
        p_value = test$p.value,
        max_contrib_value = max_contrib_val,
        max_contrib_pas = max_pas_id,
        min_contrib_value = min_contrib_val,
        min_contrib_pas = min_pas_id
      )
    } else {
      list(
        p_value = NA,
        max_contrib_value = NA,
        max_contrib_pas = NA,
        min_contrib_value = NA,
        min_contrib_pas = NA
      )
    }
  })
  ...
  chisq_df <- data.frame(
    ...
    p_value = sapply(chisq_results, function(x) x$p_value),
    max_contrib_value = sapply(chisq_results, function(x) x$max_contrib_value),
    max_contrib_pas_id = sapply(chisq_results, function(x) x$max_contrib_pas),
    min_contrib_value = sapply(chisq_results, function(x) x$min_contrib_value),
    min_contrib_pas_id = sapply(chisq_results, function(x) x$min_contrib_pas),
